...
//            ChapterExecute save = chapterExecuteRepository.save(chapter);
    ...
    public void saveChapter(Long userId, Long id, String title, String content, List<ImageInfo> imgInfo) {
        try {
            ...
            String saveDir = CloudflareR2Util.getTargetDirectoryBasedOnOS() + "chapter_" + chapter.getTrueId(); // 指定保存目录
    ...
    public UploadNovelDTO updateNovelDetail(MultipartFile cover,UploadMetadata metadata,Long novelId,Long userId) {
        try {
            ...
            if (cover != null) {
                ...
                String path = CloudflareR2Util.saveToFileSystem(cover, CloudflareR2Util.getTargetDirectoryBasedOnOS() + novelId + "/cover");
            ...
            FileUtil.deleteFolder(new File(CloudflareR2Util.getTargetDirectoryBasedOnOS() + novelId + "/cover"));
            return new UploadNovelDTO(novel, tagsByNovelId);
        } catch (Exception e) {
    ...
    public void update(MultipartFile file, Long novelId, String type, Long userId) {
        ...
        try {
            ...
            String path = CloudflareR2Util.saveToFileSystem(file, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "file");
            String extension = FileUtil.getFileExtension(file.getOriginalFilename());
            // 根据文件类型进行不同处理
            switch (Objects.requireNonNull(extension).toLowerCase()) {
                case "txt":
                    updateTxtFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "updateUploadTxtContent" + UUID.randomUUID().toString() , type, saveNovel);
                    break;
                case "zip":
                    updateSaveZipFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "updateSaveZipFile"  + UUID.randomUUID().toString() , type, saveNovel);
                    break;
                case "epub":
                    updateEpubFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "updateUploadEpubContent" + UUID.randomUUID().toString() , type, saveNovel);
                    break;
                default:
                    break;
            }
            FileUtil.deleteFolder(new File(CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId()));
        }catch (Exception e){
            throw new RuntimeException(e);
        } finally {
            userService.updateUploadByUserId(userId, false);
    ...
    public void upload(MultipartFile file, MultipartFile cover,UploadMetadata metadata, Long userId) {
        try {
            ...
            try {
                String fileName = cover.getOriginalFilename();
                String fileExtension = FileUtil.getFileExtension(fileName);
                String path = CloudflareR2Util.saveToFileSystem(cover, CloudflareR2Util.getTargetDirectoryBasedOnOS() + novel.getId() + "/uploadCoverImg" + UUID.randomUUID().toString());
            ...
            if (metadata.isUploadFile()) {
                ...
                try {
                    ...
                    switch (Objects.requireNonNull(extension).toLowerCase()) {
                        case "txt":
                            saveTxtFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "uploadTxtContent" + UUID.randomUUID().toString() , metadata.getType(), saveNovel);
                            break;
                        case "zip":
                        saveZipFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "uploadZipContent" + UUID.randomUUID().toString() , metadata.getType(), saveNovel);
                            break;
                        case "epub":
                            saveEpubFile(path, CloudflareR2Util.getTargetDirectoryBasedOnOS() + saveNovel.getId() + File.separator + "uploadEpubContent" + UUID.randomUUID().toString() , metadata.getType(), saveNovel);
                            break;
                        default:
                            break;
                    }
            ...
            FileUtil.deleteFolder(new File(CloudflareR2Util.getTargetDirectoryBasedOnOS() + novel.getId()));
        } finally {
            userService.updateUploadByUserId(userId, false);
        }
    }
