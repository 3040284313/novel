...
public class RequestLogService {
    ...
    public boolean checkRequestLimit(Credential credential, String requestURI) {
        ...
        if (requestLog == null) {
            ...
            return true;
        } else if ((CHAPTERS_ID_PATTERN.matcher(requestURI).matches() || Favorite.matcher(requestURI).matches() || limitUrl.contains(requestURI)) && requestLog.getCount() >= limitRequest) {
            return false;
        }
        if (CHAPTERS_ID_PATTERN.matcher(requestURI).matches() || Favorite.matcher(requestURI).matches()  || limitUrl.contains(requestURI)) {
